...
public class Board {
    ...
    public void addRandomTile() {
        ...
        BoardSpot randspot = openSpaces.get(randnum);


        double randperc = StdRandom.uniform(0.0, 1.0);
        if(randperc<0.1)
        {
            gameBoard[randspot.getRow()][randspot.getCol()]=4;

        }
        else
        {
            gameBoard[randspot.getRow()][randspot.getCol()]=2;
        }
    ...
    public void swipeLeft() {
        ...
        {
            int num=0;
            int[] arr = new int[gameBoard[0].length];
            for(int j=0;j<gameBoard[i].length;j++)
            {
                if(gameBoard[i][j]!=0)
                {
                    arr[num] = gameBoard[i][j];
                    num++;
                }
            }
            gameBoard[i]=arr;
    ...
    public void mergeLeft() {
        for(int i =0;i<gameBoard.length;i++)
        {
            for(int j=0;j<gameBoard[i].length-1;j++)
            {
                ...
                {
                    int ind = j+1;
                    while(gameBoard[i][j]==0 && ind<gameBoard.length)
                    {
                        ind++;
                    }
                    if(gameBoard[i][j]==gameBoard[i][ind])
                    {
                        gameBoard[i][j]=gameBoard[i][j]+gameBoard[i][ind];
                        gameBoard[i][ind]=0;
    ...
    public void transpose() {
        ...
        int[][] transarr = new int[gameBoard.length][gameBoard[0].length];

        for(int i =0;i<gameBoard.length;i++)
        {
            for(int j=0;j<gameBoard[i].length;j++)
            {
                transarr[j][i]=gameBoard[i][j];
            }
